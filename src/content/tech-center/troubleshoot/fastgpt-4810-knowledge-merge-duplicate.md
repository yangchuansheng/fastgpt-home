---
title: 解决FastGPT 4.8.10版本知识库合并组件重复触发流程问题
slug: /zh/troubleshoot/fastgpt-4810-knowledge-merge-duplicate
page_type: 故障排查
source: https://github.com/labring/FastGPT/issues/2955
source_type: GitHub issue
---

# 解决FastGPT 4.8.10版本知识库合并组件重复触发流程问题

## 适用环境与版本范围

- **环境：** FastGPT deployment environment described by the maintainer source
- **版本范围：** 维护者记录涉及 4.8.10；应用前请在目标环境确认实际 FastGPT 与相关组件版本。
- **适用边界：** 这篇说明只覆盖公开维护者来源中记录的现象。先核对部署方式、相关组件和请求入口，再执行处理步骤。

## 问题指纹

> 在FastGPT 4.8.10版本中，使用知识库合并组件合并多个知识库后，后续绑定的指定回复组件会被多次触发，知识库引用合并流程会重复执行。该问题曾在历史版本中被修复，但在4.8.10版本再次出现。
...
> 来源：[FastGPT maintainer source](https://github.com/labring/FastGPT/issues/2955)
