---
title: FastGPT 4.6.6知识库问答调用模型配置异常排查
slug: /zh/troubleshoot/fastgpt-466-model-config-troubleshooting
page_type: 故障排查
source: https://github.com/labring/FastGPT/issues/736
source_type: GitHub issue
---

# FastGPT 4.6.6知识库问答调用模型配置异常排查

## 适用环境与版本范围

- **环境：** FastGPT deployment environment described by the maintainer source
- **版本范围：** 维护者记录涉及 4.6.6；应用前请在目标环境确认实际 FastGPT 与相关组件版本。
- **适用边界：** 这篇说明只覆盖公开维护者来源中记录的现象。先核对部署方式、相关组件和请求入口，再执行处理步骤。

## 问题指纹

> 用户使用FastGPT 4.6.6私有部署版本，在知识库问答功能中出现模型调用异常：第一次回答可正常调用ChatGLM3模型，第二次回答未按照配置调用目标模型，转而调用openai模型。执行保存并预览操作后，该异常现象会重复循环出现。
...
> 来源：[FastGPT maintainer source](https://github.com/labring/FastGPT/issues/736)
