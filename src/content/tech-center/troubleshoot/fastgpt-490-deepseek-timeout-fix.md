---
title: 解决FastGPT私有部署4.9.0版本deepseek模型调用超时问题
slug: /zh/troubleshoot/fastgpt-490-deepseek-timeout-fix
page_type: 故障排查
source: https://github.com/labring/FastGPT/issues/4183
source_type: GitHub issue
---

# 解决FastGPT私有部署4.9.0版本deepseek模型调用超时问题

## 适用环境与版本范围

- **环境：** FastGPT deployment environment described by the maintainer source
- **版本范围：** 维护者记录涉及 4.9.0；应用前请在目标环境确认实际 FastGPT 与相关组件版本。
- **适用边界：** 这篇说明只覆盖公开维护者来源中记录的现象。先核对部署方式、相关组件和请求入口，再执行处理步骤。

## 问题指纹

> FastGPT私有部署4.9.0版本中，用户配置硅基流动的deepseek模型后，测试模型调用时出现request timed out报错。用户使用官方推荐的默认docker-compose-pgvector.yml配置文件，未做修改。本地ubuntu环境执行curl测试可正常连通目标API，但在Docker容器内执行相关测试时失败。
...
> 来源：[FastGPT maintainer source](https://github.com/labring/FastGPT/issues/4183)
