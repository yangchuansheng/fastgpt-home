---
title: 解决FastGPT AI PROXY中阿里云SenseVoiceSmall测试报ModelNotFound问题的排查方法
slug: /zh/troubleshoot/fastgpt-aiproxy-sensevoice-model-not-found
page_type: 故障排查
source: https://github.com/labring/FastGPT/issues/4543
source_type: GitHub issue
---

# 解决FastGPT AI PROXY中阿里云SenseVoiceSmall测试报ModelNotFound问题的排查方法

## 适用环境与版本范围

- **环境：** FastGPT deployment environment described by the maintainer source
- **版本范围：** 维护者记录涉及 4.9.4；应用前请在目标环境确认实际 FastGPT 与相关组件版本。
- **适用边界：** 这篇说明只覆盖公开维护者来源中记录的现象。先核对部署方式、相关组件和请求入口，再执行处理步骤。

## 问题指纹

> 在FastGPT 4.9.4私有部署版本中，用户通过AI PROXY添加阿里云SenseVoiceSmall模型后，点击测试按钮失败，点击问号图标显示“model not found”。后台aiproxy服务日志反复出现报错：`handle failed: &{Error:{Code:ModelNotFound Message:Model not found (SenseVoiceSmall)! Type:aiproxy_error Param:} StatusCode:50
...
> 来源：[FastGPT maintainer source](https://github.com/labring/FastGPT/issues/4543)
