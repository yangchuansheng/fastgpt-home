---
title: 解决FastGPT 4.8.16版本对话日志页面崩溃的问题
slug: /zh/troubleshoot/fastgpt-4816-log-page-crash
page_type: 故障排查
source: https://github.com/labring/FastGPT/issues/3607
source_type: GitHub issue
---

# 解决FastGPT 4.8.16版本对话日志页面崩溃的问题

## 适用环境与版本范围

- **环境：** FastGPT deployment environment described by the maintainer source
- **版本范围：** 维护者记录涉及 4.8.16；应用前请在目标环境确认实际 FastGPT 与相关组件版本。
- **适用边界：** 这篇说明只覆盖公开维护者来源中记录的现象。先核对部署方式、相关组件和请求入口，再执行处理步骤。

## 问题指纹

> FastGPT 4.8.16版本中，在对话记录页面查看日志库时页面会直接崩溃。执行`docker logs fastgpt -f`命令无日志输出，用户未发现配置文件存在错误，未使用苹果系统，未开启浏览器翻译功能。浏览器控制台出现`Minified React error #310`报错，同时包含`useMemo`相关的调用栈信息，另有`ms-high-contrast-adjust`的弃用警告。
...
> 来源：[FastGPT maintainer source](https://github.com/labring/FastGPT/issues/3607)
