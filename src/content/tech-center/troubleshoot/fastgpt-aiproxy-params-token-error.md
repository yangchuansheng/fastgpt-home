---
title: 解决FastGPT使用aiproxy时的接口参数与token异常问题
slug: /zh/troubleshoot/fastgpt-aiproxy-params-token-error
page_type: 故障排查
source: https://github.com/labring/FastGPT/issues/5055
source_type: GitHub issue
---

# 解决FastGPT使用aiproxy时的接口参数与token异常问题

## 适用环境与版本范围

- **环境：** FastGPT deployment environment described by the maintainer source
- **版本范围：** 维护者记录涉及 4.9.12、v0.2.2；应用前请在目标环境确认实际 FastGPT 与相关组件版本。
- **适用边界：** 这篇说明只覆盖公开维护者来源中记录的现象。先核对部署方式、相关组件和请求入口，再执行处理步骤。

## 问题指纹

> FastGPT 4.9.12私有部署版本，搭配aiproxy v0.2.2使用时，调用接口出现异常。调用时页面返回参数相关的错误提示；当不填写base_url配置项时，系统提示invalid_token。但使用postman直接调用同一接口与对应token时，调用可正常完成，无报错反馈。
...
> 来源：[FastGPT maintainer source](https://github.com/labring/FastGPT/issues/5055)
