---
title: 解决FastGPT API调用时仅获取工作流指定返回内容的问题
slug: /zh/troubleshoot/fastgpt-api-get-specified-workflow-output
page_type: 故障排查
source: https://github.com/labring/FastGPT/issues/3545
source_type: GitHub issue
---

# 解决FastGPT API调用时仅获取工作流指定返回内容的问题

## 适用环境与版本范围

- **环境：** FastGPT deployment environment described by the maintainer source
- **版本范围：** 维护者记录涉及 V4.8.5；应用前请在目标环境确认实际 FastGPT 与相关组件版本。
- **适用边界：** 这篇说明只覆盖公开维护者来源中记录的现象。先核对部署方式、相关组件和请求入口，再执行处理步骤。

## 问题指纹

> 用户在FastGPT V4.8.5私有部署版本中，创建包含多步骤的工作流，通过API调用应用时，返回了工作流全部步骤的输出内容，没有仅返回用户期望的第三部分指定返回结果。用户提供的Python调用代码中，请求携带了chatId、stream、detail、messages等参数，调用后返回的内容包含了工作流所有步骤的输出。
...
> 来源：[FastGPT maintainer source](https://github.com/labring/FastGPT/issues/3545)
