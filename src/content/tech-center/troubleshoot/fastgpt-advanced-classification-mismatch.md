---
title: 解决FastGPT私有部署版高级编排问题分类命中不符问题
slug: /zh/troubleshoot/fastgpt-advanced-classification-mismatch
page_type: 故障排查
source: https://github.com/labring/FastGPT/issues/1342
source_type: GitHub issue
---

# 解决FastGPT私有部署版高级编排问题分类命中不符问题

## 适用环境与版本范围

- **环境：** FastGPT deployment environment described by the maintainer source
- **版本范围：** 维护者记录涉及 v4.8-preview2；应用前请在目标环境确认实际 FastGPT 与相关组件版本。
- **适用边界：** 这篇说明只覆盖公开维护者来源中记录的现象。先核对部署方式、相关组件和请求入口，再执行处理步骤。

## 问题指纹

> 用户使用私有部署v4.8-preview2版本的FastGPT，在高级编排功能中配置了问题分类列表：账单查询、业务办理、便民服务、其他问题。当向系统发送测试文本"你是谁"时，预期命中的分类应为"其他问题"，但实际从系统日志中查看到的命中分类与预期结果不符，且已确认分类配置本身正确无误。
...
> 来源：[FastGPT maintainer source](https://github.com/labring/FastGPT/issues/1342)
