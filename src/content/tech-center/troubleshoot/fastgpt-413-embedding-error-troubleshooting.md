---
title: 解决FastGPT知识库索引时出现413状态码报错问题
slug: /zh/troubleshoot/fastgpt-413-embedding-error-troubleshooting
page_type: 故障排查
source: https://github.com/labring/FastGPT/issues/4030
source_type: GitHub issue
---

# 解决FastGPT知识库索引时出现413状态码报错问题

## 适用环境与版本范围

- **环境：** FastGPT deployment environment described by the maintainer source
- **版本范围：** 维护者记录涉及 4.9、v1.5；应用前请在目标环境确认实际 FastGPT 与相关组件版本。
- **适用边界：** 这篇说明只覆盖公开维护者来源中记录的现象。先核对部署方式、相关组件和请求入口，再执行处理步骤。

## 问题指纹

> 用户使用FastGPT私有部署4.9版本，采用Docker标准部署模式，配置硅基流动的BAAI/bge-large-zh-v1.5索引模型与BAAI/bge-reranker-v2-m3重排模型。在使用问答对提取模式上传PDF、MD文件建立知识库时，检索功能一直处于加载中，日志出现两次`413 status code (no body)`报错，分别归类为Embedding Error与Vector Queue Error。
...
> 来源：[FastGPT maintainer source](https://github.com/labring/FastGPT/issues/4030)
