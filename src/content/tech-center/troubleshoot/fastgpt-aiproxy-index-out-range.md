---
title: 解决FastGPT公有云qwen2.5-32b-128k模型调用工具时报索引越界错误
slug: /zh/troubleshoot/fastgpt-aiproxy-index-out-range
page_type: 故障排查
source: https://github.com/labring/FastGPT/issues/3885
source_type: GitHub issue
---

# 解决FastGPT公有云qwen2.5-32b-128k模型调用工具时报索引越界错误

## 适用环境与版本范围

- **环境：** FastGPT deployment environment described by the maintainer source
- **版本范围：** 维护者记录涉及 2.5-32b；应用前请在目标环境确认实际 FastGPT 与相关组件版本。
- **适用边界：** 这篇说明只覆盖公开维护者来源中记录的现象。先核对部署方式、相关组件和请求入口，再执行处理步骤。

## 问题指纹

> 使用FastGPT公有云版本，在简易应用中配置qwen2.5-32b-128k模型并挂载两个工具（如获取时间插件、谷歌搜索或DuckDuckGo搜索插件），提问“告诉我当前最新时间，并且查询这个时间的新闻”时，触发报错。报错的详细信息为： ```json { "message": "500 <500> InternalError.Algo: list index out of range (aiproxy: 17404706338861885) (request id: 20
...
- [FastGPT maintainer source](https://github.com/labring/FastGPT/issues/3885)
