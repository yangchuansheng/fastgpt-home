...
| SDK集成 | 高 | 低 | 高 | 需要深度整合AI能力到产品内核的场景，可复用SDK封装的底层逻辑 | 支持对应SDK的开发语言 | 全栈开发人员 | 可正常调用SDK接口，AI能力与原有业务逻辑无冲突 |

其中，API调用与SDK集成均支持流式响应，可实现AI内容的实时输出，适合需要即时交互的对话类场景。iframe方式虽无需代码改造，但自定义空间有限，仅适用于快速验证或轻量集成场景，无法满足深度定制的业务需求。
...
支持OpenAI SDK兼容的集成方式，可大幅降低代码改造的成本。开发者可直接复用原有基于OpenAI SDK的代码，仅需替换API端点与鉴权密钥即可接入目标AI服务，无需修改业务逻辑。具体改造步骤如下表：
...
FastGPT支持插件系统，可将集成后的AI能力封装为MCP Server，供其他内部服务或第三方服务调用。具体流程为：先基于FastGPT的插件开发规范完成AI能力的定制，例如自定义知识库解析逻辑、调整agent-loop规则或集成第三方工具；再将其发布为MCP Server，配置服务的访问协议与端口；其他产品可通过标准协议调用该服务，实现AI能力的跨服务复用。该方式可降低多产品集成AI能力的重复开发成本，提升团队的协作效率。落地责任人包括插件开发、运维与架构人员，验证标准为其他服务可正常调用MCP Server，自定义插件逻辑生效，服务可用性符合业务要求。该扩展能力符合FastGPT插件系统的规划，后续还将支持插件集成知识库来源、文件解析方式等更多能力，可提前适配后续功能更新。
