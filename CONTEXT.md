...
**交互模块页**:
正文中嵌入一个交互模块标记的技术页；读者可在页面上调整输入并看到对应结果，脚本不可用时页面仍以静态兜底表完整呈现。
_Avoid_: 工具页, 计算器页, 动态页

**交互模块标记**:
正文中声明交互模块类型、数据文件与兜底表位置的单行注释（`<!-- fastgpt-interactive: <类型> | data: <数据文件> | fallback-table-below -->`）。
_Avoid_: 组件标签, 占位符, 嵌入点

**参数模拟器**:
交互模块类型之一；读者选定处理方式与设置方式后，模块给出每个参数实际生效的取值、被改写的项与估算数量。
_Avoid_: 配置器, 参数表单

**查询器**:
交互模块类型之一；读者按码值或部署条件筛选，模块给出匹配条目，并对未输入、无匹配与跨模块重叠三种情况分别呈现。
_Avoid_: 搜索框, 过滤器

**静态兜底表**:
交互模块页中承载该模块全部数据依据的 Markdown 原生表格，模块未渲染时保持页面完整可读。
_Avoid_: 备用表格, 降级视图
