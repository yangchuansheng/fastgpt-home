...
function isCapacityReportReady(report) {
  return (
    ...
    JSON.stringify(report.variants?.map(({ variant }) => variant)) === JSON.stringify(VARIANTS) &&
...
function validateCapacityReport(report, repoRoot) {
  ...
  const expectedBlockers = deriveCapacityBlockers(report).sort();
